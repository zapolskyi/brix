...
/**
 * Множина числівника: три форми для української, дві для англійської.
 *
 * `_n()` вибирає форму за правилом мови перекладу, а української
 * .po в темі немає — вона тут мова оригіналу. Тож для «24 пачки»
 * доводиться рахувати форму самим.
 *
 * Англійською три форми теж приходять — уже перекладені, де друга й
 * третя однакові. Але правило до них треба англійське: за українським
 * «21» дало б «21 bag» замість «21 bags».
 *
...
function brix_plural( int $number, string $one, string $few, string $many ): string {
	...
	if ( function_exists( 'brix_is_en' ) && brix_is_en() ) {
		return 1 === $number ? $one : $many;
	}
